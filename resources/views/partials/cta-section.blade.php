    ...
    <div class="max-w-[1280px] mx-auto px-4 md:px-8 lg:px-16">
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16">
            <div class="flex flex-col justify-center">
                <span class="inline-block text-xs font-medium text-primary-foreground/70 uppercase tracking-widest mb-3">Sazināties</span>
                <h2 class="text-2xl md:text-3xl lg:text-4xl font-semibold text-primary-foreground tracking-tight mb-6">Pieprasīt tehnisko piedāvājumu</h2>
                <p class="text-base md:text-lg text-primary-foreground/80 mb-8 leading-relaxed">Aprakstiet projekta apjomu — sagatavosim tehnisko risinājumu un izmaksu aprēķinu.</p>
                <div class="space-y-4 mb-8">
                    <div class="flex items-center gap-3">
                        <div class="w-2 h-2 rounded-full bg-primary-foreground/60"></div>
                        <span class="text-sm text-primary-foreground/90">Fasādes projekta aprēķins</span>
                    </div>
                    <div class="flex items-center gap-3">
                        <div class="w-2 h-2 rounded-full bg-primary-foreground/60"></div>
                        <span class="text-sm text-primary-foreground/90">Stikla konstrukciju specifikācija</span>
                    </div>
                    <div class="flex items-center gap-3">
                        <div class="w-2 h-2 rounded-full bg-primary-foreground/60"></div>
                        <span class="text-sm text-primary-foreground/90">Pilna cikla būvniecības piedāvājums</span>
                    </div>
                </div>
            ...
            <div>
                @if(session('success'))
                    <div class="bg-white/10 border border-white/20 text-white px-6 py-4 rounded-xl relative mb-6 backdrop-blur-sm" role="alert">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                <strong class="font-bold text-lg block mb-0.5">Paldies!</strong>
                ...
                @if(session('error'))
                    <div class="bg-white/10 border border-white/20 text-white px-6 py-4 rounded-xl relative mb-6 backdrop-blur-sm" role="alert">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                <strong class="font-bold text-lg block mb-0.5">Kļūda!</strong>
                ...
                @if ($errors->any())
                    <div class="bg-white/10 border border-white/20 text-white px-6 py-4 rounded-xl relative mb-6 backdrop-blur-sm" role="alert">
                         <div class="flex items-start gap-3">
                            ...
                            <div>
                                <strong class="font-bold text-lg block mb-1">Lūdzu, pārbaudiet datus!</strong>
                ...
                <form x-data="{ isSubmitting: false }" @submit="isSubmitting = true" action="{{ route('contact.store') }}" method="POST" class="bg-background rounded-2xl p-6 md:p-8 shadow-2xl">
                    ...
                    <div class="grid grid-cols-1 gap-4 md:gap-6 mb-4 md:mb-6">
                        <div class="space-y-2">
                            <label for="contact" class="text-sm font-medium text-foreground">Kontaktpersona <span class="text-red-500">*</span></label>
                            <input id="contact" name="contact" required placeholder="Jūsu vārds" value="{{ old('contact') }}" class="h-11 w-full rounded-md border border-input bg-muted/50 px-3 py-2 text-sm ring-offset-background file:border-0 file:bg-transparent file:text-sm file:font-medium placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50 transition-colors">
                        </div>
                        <div class="space-y-2">
                            <label for="phone" class="text-sm font-medium text-foreground">Tālrunis <span class="text-red-500">*</span></label>
                    ...
                    <button type="submit" :disabled="isSubmitting" class="w-full inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-md text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 h-11 px-8 hover:bg-primary/90 bg-primary text-primary-foreground">
                        <span x-show="!isSubmitting" class="flex items-center gap-2">
                             Nosūtīt pieprasījumu <i data-lucide="arrow-right" class="w-4 h-4"></i>
                        </span>
